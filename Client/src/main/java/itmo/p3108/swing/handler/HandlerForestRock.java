...
public class HandlerForestRock extends MapHandler {
    private static final MapHandler handler = new HandlerForestEverest();

    @Override
    public void moveElement(Person person, JLabel label) {
        if (person.getResp().equals(Place.FOREST) && person.getTargetPlace().equals(Place.ROCK)) {
            for (var i = -1; i > -10; i--) {
                int z = 1;
                while (z != Math.abs(i)) {
                    sleep(50);
